...
### In case the model get stuck in infeasibility, we can use RelaxBalanceUp and RelaxBalanceDown to relax the balance constraint (with some cost)
...
#     m.RelaxBalanceDown = Var(
#         m.GAS_ZONES_TIMESERIES, 
#         within=NonNegativeReals) #free disposal of gas
...
#             # Set high price (ex. as much as $15/MMBtu) so the demand (that exceed supply) will be switched to other source of energy or get more imports
#             15 * (m.RelaxBalanceUp[z, ts])
